...
class MemberPlanForm(forms.ModelForm):
    is_active = forms.ChoiceField(choices=[(True, 'Yes'), (False, 'No')], widget=widgets.RadioSelect(attrs={"class": ""}), required=False)
    is_paid = forms.ChoiceField(choices=[(True, 'Yes'), (False, 'No')], widget=widgets.RadioSelect(attrs={"class": ""}), required=False)
    class Meta:
        model = MemberPlan
        fields = ['is_active', 'is_paid', 'member', 'plan', 'startdate']
        widgets = {
